<div class="people index">
	...
	<fieldset>
		...
		<?php echo $this->Html->link('<Back', '#', array('class' => 'button', 'id' => 'back-month')); ?>
		<?php echo $this->Html->link('Next>', '#', array('class' => 'button', 'id' => 'next-month')); ?>
	</div>
	<div class="filter">
		<?php echo $this->Form->create('Membership', array(
			'class' => 'addFilterMonth',	
			'action' => '',
		)); ?>
	<fieldset>
		...
		<?php echo $this->Form->end(__('Submit'));?>
		<?php echo $this->Html->link('Clear', 'list_by_month/' . $niceYear . "/" . $niceMonth. "/$", array('class' => 'button', 'id' => 'clear-button')); ?>
	</div>
	<h2><?php echo __('Connections for ') . $monthName .', '. $niceYear . ' ' . $memberCheck; ?></h2>
